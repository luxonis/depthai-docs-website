...
If you're reading this guide, you most likely just got your first OAK camera (e.g. `OAK-D <https://docs.luxonis.com/projects/hardware/en/latest/pages/BW1098OAK.html>`__). Welcome!
In what follows, we'll help you learn how to set up your OAK and begin to explore everything possible you can achieve with it.

- First, we will run a DepthAI demo script, which will allow you to preview DepthAI functionalities.
- Next up, we will go through some of the options available in the demo script and show their use/results.
- Last, you will receive useful links to expand your knowledge further and check open-sourced use-case implementations, code examples, and tutorials, which you can then begin to use as a starting point for your projects.
...
Make sure that the device is connected to your host (which can be a PC, Raspberry Pi, or another capable device) directly to a USB port,
...
.. note::
  If you are running an OS other than Ubuntu, or you think something has gone wrong, we have detailed OS-specific installation guides.
  `here <https://docs.luxonis.com/projects/api/en/latest/install/#supported-platforms>`__.

Connect the OAK camera (PoE)
############################

If you are using a PoE device, you should follow the `Getting started with OAK PoE devices <https://docs.luxonis.com/projects/hardware/en/latest/pages/guides/getting-started-with-poe.html>`__ tutorial.
...
In this section, we will describe how to install the demo script manually with a command line.
...
and then unpack the archive to a directory of your preference. Next, open a terminal session in this directory.

From git
--------

First, open the terminal session and go to a directory of your preference where you'd like to download your demo script.
...
After the repository is downloaded, make sure to enter the downloaded repository by running the following:
...
This will make sure that you are using a fresh environment and that Python 3 is the default interpreter, which can help to prevent potential issues.

We typically recommend creating and using virtualenvs by running the following:
...
This may require installing these packages first:
...
is install all additional packages that this script requires (together with the :code:`depthai` Python API itself).
...
.. warning::
  ...
  the device correctly. You can add and apply new rules by running the following:
...
Now, you should be able to start using the demo script, which we will now proceed to.

Run demo script
***************

Having everything set up, we are now ready to use the demo script by running this:
...
While the demo was running, you should see the detection results. If you were standing in front of the camera,
...
This will allow you to detect human faces, like below:


.. image:: https://user-images.githubusercontent.com/5244214/142645887-f04b90e4-c69f-4764-93d3-37bfb05fe67c.png
  :alt: face

You can use this combo box to change the model that is being run on the DepthAI. It's also possible to use a command line to select:
...
- :code:`pedestrian-detection-adas-0002` - Allows detection of people on the image (slower)
...
- :code:`person-detection-retail-0013` - Allows detection of people on the image (faster)
...
- :code:`person-vehicle-bike-detection-crossroad-1016` - Allows detection of people, bikes, and vehicles on the image
...
- :code:`vehicle-detection-adas-0002` - Allows detection of vehicles on the image
...
- :code:`vehicle-license-plate-detection-barrier-0106` - Allows detection of both vehicles and license plates on the image (only Chinese license plates)
...
- **Enabled**: Turns AI on/off. Turning it off will prevent any neural network from being run, which will also save some of the memory. This is useful for situations where we're more focused on Depth/Encoding rather than AI processing.

Basic properties:

- **CNN Model**: Choose which model to run on DepthAI, see more at :ref:`Using other models`
- **SHAVEs**: Determines the number of SHAVE cores used to compile the neural network. The higher the value, the faster network can run, but this also limits features that can be enabled simultaneously.
...
we'll add a custom face detection network called :code:`custom_model` (substitute with your preferred name)
...
You can check how this file looks `in OpenVINO model zoo <https://github.com/openvinotoolkit/open_model_zoo/blob/master/models/intel/face-detection-retail-0004/model.yml>`__
...
If you'd like to add some custom functionalities to the demo yourself or just check what certain variables look like,
...
If preferred, the Demo script can also be imported like a regular class. This allows us to control when the demo is starts up
and when it runs. A simple example of how to run the demo from Python code is shown below:
...
In the previous sections, we learned how to preview basic DepthAI features. From this point, you can explore the DepthAI world farther.
