...
installed you will also need to install `opencv-python
<https://pypi.org/project/opencv-python/>`__.
...
We'll use :code:`opencv` to display the video stream and :code:`depthai` to
access the camera and its data packets. To verify all of these dependencies are
installed correctly add the following to a Python source file and execute it.
